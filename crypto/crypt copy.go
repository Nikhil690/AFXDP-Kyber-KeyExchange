...
func StartHelloNew(xsk *xdp.Socket, publicKey kem.PublicKey, packet gopacket.Packet) {
	...
	if err != nil {
		log.Fatal("Failed to marshal public key message:", err)
	}
	...
	// Create structured message using the Message struct
	msg := &message.Message{
		Type:    message.MsgTypeKeyExchange,
		Length:  uint32(len(keyMsgBytes)),
		Payload: keyMsgBytes,
	}

	// Serialize the message to binary format using the struct method
	httpPayload := msg.Serialize()
